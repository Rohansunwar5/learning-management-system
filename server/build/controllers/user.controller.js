...
const createActivationToken = (user) => {
    ...
    // Use activationCode in your activation process
...
exports.loginUser = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        if (!(email || password)) {
            return next(new ErrorHandler_1.default("Please enter your email and password", 401));
        }
        const user = await user_model_1.default.findOne({ email }).select("+password"); // +password so that mongoose send the other fields also in the document
        if (!user) {
...
exports.updateAccessToken = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        await redis_1.redis.set(user._id, JSON.stringify(user), "EX", 604800); // 7 days expiry
        next();
        // send response
        // res.status(200).json({
        //   status: "success",
...
exports.updatePassword = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        // user?.password = newPassword; Since we are taking the user from social auth also we cannot procced in this pattern
    ...
    try {
        ...
        if (avatar && user) {
            ...
            if (user?.avatar?.public_id) {
                // first delete the old image and upload the new one
